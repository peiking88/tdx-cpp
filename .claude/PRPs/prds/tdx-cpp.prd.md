...
_Status: ARCHITECTURE REVIEWED — 架构评审通过；评审 P0/P1 反馈已采纳并修复（Retry/CircuitBreaker 前移 Phase 1、dragonfly 分层采纳、fiber 纪律入规范、测试策略补强）_
...
- **假设**：C++ 版「被 C++ 量化系统实际采用」—— _需通过首个集成方验证_。
...
| --- | ------------------------ | ----------------------------------------------------------------------------------------------- | ----------- | -------- | ------- | --------------------------------------------- |
| 1   | 协议层 + A股标准行情核心 | helio 连接/帧/解析器注册表/编解码/心跳/选服/**Retry+CircuitBreaker** + StdQuotes + VipdocReader | complete    | -        | -       | [plan](phase1-protocol-layer-a-share.plan.md) |
| 2   | 扩展市场 + SP高级行情    | exQuotationClient/macQuotationClient + ExtQuotes + SP协议 + 板块                                | complete    | 评估     | 1       | [plan](phase2-extended-sp-quotes.plan.md)     |
| 3   | 数据管理层（功能对等）   | TdxData统一API + Sync + Hybrid + 复权 + 时段重采样 + Calendar + config.json缓存                 | in-progress | 评估     | 1       | [plan](phase3-data-manager.plan.md)           |
| 4   | 改进点（v2，已推迟）     | 并发批量(-n) + 股票级断点续传 + 落盘存储(dragonfly热缓存+Parquet冷存储)                         | deferred    | -        | 3       | [plan](phase4-v2-improvements.plan.md)        |
...
- **Scope**：
  ...
  - **落盘存储（分层）**：
    ...
    - _注：dragonfly 纯内存 Redis 兼容，KV 模型不适合 K线范围查询，故仅做热缓存；冷存储用列式文件。_
...
_评审历史：`/home/li/peiking88/tdx-cpp/.claude/PRPs/reviews/tdx-cpp.architecture-review.md`（REQUEST CHANGES → 已按用户 5 条决策修复 P0/P1）_
_下一步：评审黄金字节流方案 `phase1-golden-byte-stream.md` → 运行 `/prp-plan` 生成 Phase 1 实施计划。_
