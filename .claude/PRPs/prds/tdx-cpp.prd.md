...
<<<<<<< Updated upstream
| 3 | 数据管理层（功能对等） | TdxData统一API + Sync + Hybrid + 复权 + 时段重采样 + Calendar + config.json缓存 | in-progress | 评估 | 1 | - |
=======
| 3 | 数据管理层（功能对等） | TdxData统一API + Sync + Hybrid + 复权 + 时段重采样 + Calendar + config.json缓存 | pending | 评估 | 1 | [plan](phase3-data-manager.plan.md) |
>>>>>>> Stashed changes
