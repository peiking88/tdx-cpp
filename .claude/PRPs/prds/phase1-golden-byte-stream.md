...
> _Generated: 2026-06-22 | Status: DRAFT — 待评审_
...
- **待确认**：上游 0x0f 的 parser 文件位置（grep 显示 xdxr 出现在 company_info.py 与 **init**.py，可能内联于 client 而非独立 parser）。
...
_评审通过后，本方案作为 Phase 1 实施计划（`/prp-plan`）的输入之一，golden 测试用例纳入 Phase 1 Success Signal ③。_
