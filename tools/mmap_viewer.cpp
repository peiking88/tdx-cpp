...
static std::vector<std::string> LoadBlk(const char *path)
{
  ...
  if (!f.is_open())
  {
    std::fprintf(stderr, "无法打开自选股文件: %s\n", path);
    return codes;
  }
  std::string line;
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    if (std::strcmp(argv[i], "--shm") == 0 && i + 1 < argc)
      shm_path = argv[++i];
    else if (std::strcmp(argv[i], "--blk") == 0 && i + 1 < argc)
      blk_path = argv[++i];
  ...
  if (!seg)
  {
    std::fprintf(stderr, "无法打开共享内存: %s（请先启动 fetch-quotes --mmap_path %s）\n",
                 shm_path.c_str(), shm_path.c_str());
    return 1;
  }
  ...
  while (true)
  {
    ...
    std::printf(" %-8s %10s %10s %10s %10s %15s %15s %12s\n",
    ...
    for (const auto &c : codes)
    {
      ...
      double chg_pct = 0;
      if (q.pre_close > 0)
        chg_pct = (q.price - q.pre_close) / q.pre_close * 100.0;

      // 涨跌色
      const char *color = "\033[0m";
      if (chg_pct > 0.01)
        color = "\033[31m"; // 红涨
      else if (chg_pct < -0.01)
        color = "\033[32m"; // 绿跌
      const char *reset = "\033[0m";

      std::printf(" %s%-8s%s %8.2f %8.2f %8.2f %8.2f %12.0f %13.0f %5.1f%%\n",
                  color, c.c_str(), reset,
