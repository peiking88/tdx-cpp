...
- **v0.15.2**：fetch-quotes 三处修复——①`[skip]` 去重失效（`seen` 声明在 if 块内每迭代新建空 set，`insert().second` 恒 true → 重复刷屏；移到 for 循环外，IngestChunk + RunOneRound 阶段B 两处）；②unusual 市场级数据随 worker0 批次数重复拉取（移到 `for(bi)` 外只拉一次）；③异步路径 quote 计数含 skipped（`cnt.quote` 从 `ch.quotes.size()` 改为 `Put` 后 `++`，与同步路径 `InsertQuote` 语义一致）
